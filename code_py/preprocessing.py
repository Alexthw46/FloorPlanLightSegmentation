...
def pregenDataAugm(input_img_paths, target_img_paths, in_path, tg_path):
    augmentation = abm.Compose([
        abm.RandomRotate90(p=0.3),
        abm.HorizontalFlip(p=0.3),
        abm.VerticalFlip(p=0.3)
    ...
    for input, target in input_img_paths, target_img_paths:
        image = Image.open(os.path.join(in_path, input))
        mask = Image.open(os.path.join(in_path, target))
